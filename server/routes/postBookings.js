...
// Medical Bookings
router.post("/new-medicalbooking", async (req, res) => {
  try {
    const {
      fullName,
      email,
    ...
    // Save booking in the database
    const newBooking = await MedicalBookings.create({
      fullName,
      email,
      address,
      zipCode,
    ...
    try {
      const emailData = {
        ...
        html: `<h2>New Booking Details</h2>
            ...
            <p><strong>Date of Birth:</strong> ${newBooking.dob}</p>
            <p><strong>Email:</strong> ${newBooking.email}</p>
            <p><strong>Address:</strong> ${newBooking.address}</p>
            <p><strong>ZipCode:</strong> ${newBooking.zipCode}</p>
...
router.get("/confirm-medicalbooking/:id", async (req, res) => {
  try {
    const booking = await MedicalBookings.findById(req.params.id);
    if (!booking) {
      return res.status(404).send("Booking not found");
    }
    if (booking.confirmed) {
        ...
        const response = await Promise.all(
          batch.map((email) =>
            mailgun.messages.create("motgpayment.com", {
              ...
              to: email,
              subject: "Booking is Ready for Processing",
              html: `<h2>Medical Booking is Ready</h2>
